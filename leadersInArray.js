 ...
 * Example 1:
    ...
    * Explanation: The first leader is 17 as it is greater than all the elements to its 
        * right. Similarly, the next leader is 5. The right most element is always a leader 
...
function leaders(arr, n){
    ...
    for(let i=n-2;i>=0;i--) {
        if(arr[i] >= max) {
            max = arr[i];
            op.push(arr[i]); // unshift can also be used here. But it is giving timeout error.
        }
    }
